...
contract AttestationValidator is Initializable, OwnableUpgradeable {
    ...
    function validateAttestationDirect(
        ...
        // Use referenceBlock + task.policyConfig.expireAfter for expiration (matching respondToTask logic)
        // For direct validation, referenceBlock is the current block (when respondToTask would have been called)
        uint32 referenceBlock = uint32(block.number);
        uint32 expiration = referenceBlock + task.policyConfig.expireAfter;
        ...
        NewtonMessage.Attestation memory attestationToCheck = NewtonMessage.Attestation(
            taskId, task.policyId, task.policyClient, expiration, task.intent, task.intentSignature
        );
