...
contract MetaCoin is Ownable, NewtonProtected {
    ...
    constructor(address _owner, address _newtonProxyAddress) Ownable(_owner) {
        balances[_owner] = 10_000_000_000_000;
        _setNewtonProxy(_newtonProxyAddress);
    }

    function sendCoin(address _sender, address _receiver, uint256 _amount) external payable onlyNewtonProxy {
    ...
    function enableNewtonProxy() external onlyOwner {
        _enableNewtonProxy();
    }

    function disableNewtonProxy() external onlyOwner {
        _disableNewtonProxy();
    }
}
