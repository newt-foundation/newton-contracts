...
library SetupDistributionsLib {
    ...
    function createAvsRewardsSubmissions(
        address incProverServiceManager,
        address strategy,
        uint256 numPayments,
        ...
        for (uint256 i = 0; i < numPayments; i++) {
            ...
            strategiesAndMultipliers[0] = IRewardsCoordinatorTypes.StrategyAndMultiplier({
                strategy: IStrategy(strategy), multiplier: 10000
            });
        ...
        ECDSAServiceManagerBase(incProverServiceManager)
            .createAVSRewardsSubmission(rewardsSubmissions);
    }
    ...
    function createOperatorDirectedAvsRewardsSubmissions(
        ...
        for (uint256 i = 0; i < operators.length; i++) {
            operatorRewards[i] = IRewardsCoordinatorTypes.OperatorReward({
                operator: operators[i], amount: operatorRewardAmount
            });
        }
    ...
    function processClaim(
        ...
        uint32[] memory tokenIndices = new uint32[](numTokenEarnings);
        bytes[] memory tokenProofs = new bytes[](numTokenEarnings);
        tokenProofs[0] = tokenProof;

        IRewardsCoordinatorTypes.TokenTreeMerkleLeaf[] memory tokenLeaves =
            new IRewardsCoordinatorTypes.TokenTreeMerkleLeaf[](numTokenEarnings);
        ...
        // Safely cast rootIndex and indexToProve to uint32 with overflow checks
        require(rootIndex <= type(uint32).max, "Root index too large for uint32");
        require(indexToProve <= type(uint32).max, "Index too large for uint32");
    ...
    function submitRoot(
        ...
        bytes32 paymentRoot = createPaymentRoot(
            rewardsCoordinator, tokenLeaves, earnerLeaves, numPayments, numTokenEarnings, filePath
        );
    ...
    function createPaymentRoot(
        ...
        require(
            earnerLeaves.length == numPayments, "Number of earners must match number of payments"
        );
        bytes32[] memory leaves = new bytes32[](numPayments);

        require(
            tokenLeaves.length == numTokenEarnings,
    ...
    function createEarnerLeaves(
        ...
        for (uint256 i = 0; i < earners.length; i++) {
            leaves[i] = IRewardsCoordinatorTypes.EarnerTreeMerkleLeaf({
                earner: earners[i], earnerTokenRoot: createTokenRoot(tokenLeaves)
            });
        }
        return leaves;
    }
    ...
    function createTokenLeaves(
        ...
        bytes32[] memory leaves = new bytes32[](numTokenEarnings);
        for (uint256 i = 0; i < numTokenEarnings; i++) {
            IRewardsCoordinatorTypes.TokenTreeMerkleLeaf memory leaf =
                defaultTokenLeaf(tokenEarnings, strategy);
    ...
    function defaultTokenLeaf(
        ...
        IRewardsCoordinatorTypes.TokenTreeMerkleLeaf memory leaf =
            IRewardsCoordinatorTypes.TokenTreeMerkleLeaf({
                token: IStrategy(strategy).underlyingToken(), cumulativeEarnings: tokenEarnings
            });
        return leaf;
    }
    ...
    function writeLeavesToJson(
        ...
        string memory parentObject = "parent_object";
        VM.serializeBytes32(parentObject, "leaves", leaves);
        string memory finalJson = VM.serializeBytes32(parentObject, "tokenLeaves", tokenLeaves);
