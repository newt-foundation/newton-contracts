...
library CoreDeploymentLib {
    ...
    function deployContracts(
        ...
        uint32 calculationIntervalSeconds = 1 days;
        uint32 maxRewardsDuration = uint32(configData.rewardsCoordinator.maxRewardsDuration);
        uint32 maxRetroactiveLength = uint32(configData.rewardsCoordinator.maxRetroactiveLength);
        uint32 maxFutureLength = 1 days;
        uint32 genesisRewardsTimestamp = 10 days;
        IRewardsCoordinatorTypes.RewardsCoordinatorConstructorParams memory
    ...
    function readDeploymentConfigValues(
        ...
        require(VM.exists(pathToFile), DeploymentFileDoesNotExist());
    ...
    function readDeploymentJson(
        ...
        return _readDeploymentJson(
            "script/deployments/core/", string.concat(VM.toString(chainId), "-", env, ".json")
        );
    }
    ...
    function writeDeploymentJson(
        ...
        string memory env = VM.envOr("DEPLOYMENT_ENV", string("stagef"));
        writeDeploymentJson("deployments/core/", block.chainid, data, env);
    }
    ...
    function writeDeploymentJson(
        ...
        string memory fileName = string.concat(path, VM.toString(chainId), "-", env, ".json");
        if (!VM.exists(path)) {
